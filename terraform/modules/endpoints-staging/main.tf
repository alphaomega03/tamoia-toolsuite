...
resource "google_endpoints_service" "api-staging" {
  project = var.project_id
  service_name = local.endpoints_host
  openapi_config = templatefile("auth-openapi-v2.json", {
...
# Enable service
resource "google_project_service" "api-staging-project-service" {
  service = google_endpoints_service.api-staging.service_name
  project = var.project_id
  depends_on = [google_endpoints_service.api-staging]
}


# Create/update Cloud Run service
resource "google_cloud_run_service" "endpoints-staging-cloud-run" {
  ...
  template {
    spec {
      containers {
        image = "us-central1-docker.pkg.dev/${var.project_id}/nodejs-containers-staging/basic-express-container:latest"
        env {
...
resource "google_cloud_run_service_iam_policy" "noauth-endpoints" {
  location    = google_cloud_run_service.endpoints-staging-cloud-run.location
  project     = google_cloud_run_service.endpoints-staging-cloud-run.project
  service     = google_cloud_run_service.endpoints-staging-cloud-run.name
